...
### 修复
- **数据库管理器初始化逻辑重构**：移除自动降级机制，统一错误处理流程
  - 移除 `get_db_manager()` 中的 MySQL 自动降级到 SQLite 逻辑，简化数据库管理器获取流程
  - 将数据库初始化错误处理从 `core/database.py` 迁移到调用方（`main.py` 和 `qa_bot.py`）
  - 在 `main.py` 和 `qa_bot.py` 中添加 MySQL 初始化失败时的回退机制
    - 自动重置 `db_manager` 为 `None`
    - 更新环境变量 `DATABASE_TYPE` 为 `sqlite`
    - 重新调用 `get_db_manager()` 获取 SQLite 管理器
    - 记录详细的降级日志信息
  - 更新 `get_db_manager()` 的文档说明，明确初始化由调用者在异步上下文中处理

### 技术实现
- **core/database.py** 更新：
  - 简化 `get_db_manager()` 函数，移除自动降级逻辑
  - 更新函数文档字符串，说明初始化责任
  - 保留单例模式，确保全局只有一个数据库管理器实例

- **main.py** 增强：
  - 在数据库初始化外层添加 try-except 错误处理
  - 捕获 MySQL 连接失败异常（如 aiomysql.OperationalError）
  - 实现自动降级到 SQLite 的逻辑
  - 添加降级后的配置更新和重新初始化流程
  - 记录详细的日志信息（尝试连接、失败原因、降级成功）

- **qa_bot.py** 增强：
  - 与 `main.py` 保持一致的错误处理机制
  - 实现 MySQL 初始化失败时的自动降级
  - 确保问答 Bot 能独立处理数据库初始化问题

### 改进效果
- **责任分离**：数据库模块负责提供管理器，调用方负责初始化和错误处理
- **灵活性提升**：调用方可以根据不同场景实现不同的降级策略
- **日志完善**：提供更清晰的错误信息和降级过程记录
- **可维护性**：代码结构更清晰，易于理解和维护

### 向后兼容
- **完全兼容**：不影响现有的数据库初始化流程
- **默认行为不变**：MySQL 连接失败时仍然会自动降级到 SQLite
- **无需配置修改**：用户无需修改任何配置文件

## [1.6.7] - 2026-02-27

### 新增
- **智能注册引导系统（异步实现）**：优化用户体验，自动引导未注册用户
  - 点击"申请周报总结"按钮时，自动检查用户是否已在 QA Bot 中注册
  - 未注册用户：编辑消息显示注册引导，包含可点击的注册按钮，跳转到 QA Bot 发送 `/start` 注册
  - 已注册用户：正常提交周报申请请求
  - 支持通过环境变量 `QA_BOT_USERNAME` 配置 QA Bot 用户名，用于生成跳转链接
  - QA Bot 的 `/start` 命令优化：首次启动时自动注册用户，后续启动显示欢迎信息
  - **智能按钮恢复机制**：
    - 每秒检查用户注册状态，注册成功后立即恢复"申请周报总结"按钮（无需等待30秒）
    - 30秒超时后无条件恢复原始按钮，确保消息正常显示
    - 未注册用户可多次尝试，每次点击都会重新触发注册引导

### 改进
- **用户体验优化**：
  - 未注册用户点击申请按钮后，看到友好的引导消息和注册按钮
  - 点击注册按钮后自动跳转到 QA Bot，简化注册流程
  - 注册完成后可正常使用周报申请功能
  
- **QA Bot 注册流程优化**：
  - 首次发送 `/start` 命令时自动注册用户
  - 后续发送 `/start` 命令时显示欢迎信息
  - 记录用户名、名字、注册时间等信息
  - 自动更新最后活跃时间

### 技术实现
- **core/channel_comment_welcome.py** 更新：
  - `handle_summary_request_callback()` 函数添加注册检查逻辑
  - 调用数据库的 `is_user_registered()` 方法检查用户注册状态
  - 未注册时显示引导消息和 URL 按钮，跳转到 QA Bot
  - 已注册时继续处理申请流程

- **qa_bot.py** 更新：
  - `start_command()` 函数优化，首次启动自动注册用户
  - 后续启动显示欢迎信息和功能说明
  - 调用数据库的 `register_user()` 方法注册新用户

- **core/config.py** 更新：
  - 新增 `QA_BOT_USERNAME` 配置项，支持从环境变量读取 QA Bot 用户名
  - 用于生成跳转链接 `https://t.me/{QA_BOT_USERNAME}`

- **core/database.py** 和数据库子类：
  - `is_user_registered()` 方法已存在，直接复用
  - 支持 SQLite 和 MySQL 双数据库

- **data/.env.example** 更新：
  - 添加 `QA_BOT_USERNAME` 配置项说明和示例

- **core/i18n.py** 更新：
  - 新增 `comment_welcome.not_registered` - 未注册提示消息
  - 新增 `comment_welcome.register_button` - 注册按钮文本
  - 支持中英文双语

### 测试
- **tests/test_comment_welcome_registration.py** - 新增单元测试
  - 测试用户未注册时的处理逻辑
  - 测试用户已注册时的处理逻辑
  - 测试无效回调数据的处理
  - 测试已存在待处理请求的情况
  - 使用 pytest-asyncio 框架，所有测试通过

### 配置示例
```bash
# 在 data/.env 中添加
QA_BOT_USERNAME=your_qa_bot_username_here
```

### 使用场景
- **场景1**：用户点击"申请周报总结"按钮，但从未使用过 QA Bot
  - 系统检测到用户未注册
  - 显示引导消息："请先在 QA Bot 中注册"
  - 显示注册按钮："前往 QA Bot 注册"
  - 用户点击按钮，跳转到 QA Bot
  - 用户发送 `/start`，自动完成注册
  - 返回讨论组，再次点击"申请周报总结"按钮
  - 系统检测到用户已注册，正常处理申请
...
- **完全兼容现有功能**：
  - 不影响任何现有的总结申请流程
  - 不影响 QA Bot 的其他功能
  - 未配置 `QA_BOT_USERNAME` 时，未注册用户仍会收到提示消息
  - 所有现有命令和配置保持不变

### 注意事项
- 用户必须先在 QA Bot 中注册才能申请周报总结
- 注册后自动更新最后活跃时间
- 数据库记录用户注册信息和活跃状态
- 支持多数据库（SQLite 和 MySQL）


## [1.6.6] - 2026-02-27

### 新增
- **频道ID标准化功能**：添加 `normalize_channel_id()` 函数，支持多种格式自动转换（@username、URL、纯数字ID），确保频道ID一致性
- **数据库自动降级功能（异步实现）**：MySQL 连接失败时自动降级到 SQLite，无需手动干预
- **命令菜单优化**：按功能逻辑重新分类为9大类（基础与核心、频道管理、定时与推送、AI配置、频道互动、统计与历史、系统运维、数据库管理、偏好设置），提升用户体验

### 修复
- **帮助命令翻译键缺失问题**：修复了 `/help` 命令中多个翻译键不存在的问题
  - 问题：使用了错误的翻译键导致帮助信息显示为键名而非实际文本
  - 修复：更新 `handle_help` 函数使用正确的翻译键
  - 新增翻译键：`help.section_prompt`（提示词管理分类）
  - 修复效果：`/help` 命令现在能正确显示所有分类的中文和英文标题

### 改进
- **代码可维护性**：命令注册代码结构清晰，新增命令时更容易找到合适位置
- **用户体验**：命令菜单按使用场景分类，危险操作单独分组，高频配置集中展示
- **数据完整性**：标准化频道ID格式，避免因格式不同导致的数据重复或丢失
- **国际化同步**：新增 19 个翻译 key（分类标题 9 个 + 数据库降级 10 个）

### 修复
- 频道ID格式不一致导致的总结时间记录和查询问题
- 数据库中因频道ID格式不同导致的记录重复问题

### 技术实现
- `core/config.py`：新增 `normalize_channel_id()` 函数
- `core/database_mysql.py`：添加连接失败检测和详细日志记录
- `core/database.py`：重构 `get_db_manager()` 函数，实现自动降级逻辑
- `core/i18n.py`：新增数据库降级和命令分类相关翻译
- `main.py`：优化命令注册代码结构，添加清晰的分组注释

### 向后兼容
- 所有命令功能保持不变，命令名称和参数不变，用户可直接使用无需学习

## [1.6.5] - 2026-02-26

### 新增
- 添加 `/update` 一键更新指令，支持自动检查 GitHub Release 版本并执行更新
  - 版本检查：对比本地与 GitHub Release 版本
  - 自动更新：git pull + 依赖安装 + 重启
  - 权限控制：仅管理员可用
  - 国际化支持：中英文双语

### 改进
- 优化版本管理，从 main.py 读取 __version__

### 修复
**频道白名单检查逻辑优化**：优化了频道白名单检查逻辑并重构配置获取
- 新增 `is_channel_in_whitelist` 函数，统一处理频道ID与白名单的匹配逻辑，支持多种格式（@username、URL、纯ID）
- 重构 `handle_new_comment` 方法，将白名单检查提前至消息处理入口，避免无效处理
- 移除冗余的频道URL匹配逻辑，直接通过频道实体获取标识符进行白名单验证
- 优化配置获取逻辑，移除对 `CHANNELS` 列表的直接依赖，改为通过频道ID获取配置
- 增强错误处理，当获取频道实体失败时使用数字ID作为备选标识符
...
- **版本控制清理**：从版本控制中移除不必要的文件
  - 移除 `coverage.json`（测试覆盖率数据文件，应由 gitignore 管理）
  - 移除 `query`（临时调试文件）
