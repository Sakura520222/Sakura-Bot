...
│   └── session_name.session      # Telegram会话文件
│
└── 📄 Docker配置
...
- **会话文件**：`bot_session.session`、`session_name.session` 持久化保存在根目录，避免重复登录
...
│
├── 📄 bot_session.session
└── 📄 session_name.session       # Telegram会话文件（运行时生成，需保密）
```
...
- **会话文件**：`bot_session.session`、`session_name.session` 需要保密，不要提交到版本控制系统
