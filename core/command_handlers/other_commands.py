...
async def handle_set_log_level(event):
    ...
    try:
        ...
        logger.info(
            f"日志级别已从 {logging.getLevelName(old_level)} 更改为 {logging.getLevelName(new_level)}"
        )
        await event.reply(
            get_text(
                "loglevel.set_success", level=level_str, old_level=logging.getLevelName(old_level)
            )
        )
    ...
    try:
        ...
        if frequency_or_day in ["daily", "weekly"]:
            ...
            if frequency == "daily":
                if len(parts) < 5:
                    await event.reply(
                        get_text("schedule.daily_need_time", channel=channel.split("/")[-1])
                    )
                    return
                ...
                success = set_channel_schedule_v2(
                    channel, frequency="daily", hour=hour, minute=minute
                )
                ...
                success = set_channel_schedule_v2(
                    channel, frequency="weekly", days=days, hour=hour, minute=minute
                )

                if success:
                    day_map = {
                        "mon": get_text("day.mon"),
                        "tue": get_text("day.tue"),
                        "wed": get_text("day.wed"),
                        "thu": get_text("day.thu"),
                        "fri": get_text("day.fri"),
                        "sat": get_text("day.sat"),
    ...
    try:
        ...
        if len(parts) > 1:
            ...
            if enabled is None:
                ...
                enabled_text = (
                    get_text("poll.status_enabled") if enabled else get_text("poll.status_disabled")
                )

            location_text = (
                get_text("poll.location_channel")
                if send_to_channel
                else get_text("poll.location_discussion")
            )
    ...
    try:
        ...
        if len(parts) < 2:
            ...
            language_info += (
                get_text(
                    "language.current",
                    language=f"{lang_names.get(current_lang, current_lang)} ({current_lang})",
                )
                + "\n\n"
            )
